...
static void updatewindowdesktop(Client *c);
...
quit(const Arg *arg)
{
	...
	dummy.window = root;
	dummy.format = 32;
	XSendEvent(dpy, root, False, SubstructureRedirectMask, (XEvent*)&dummy);
	XFlush(dpy);
}
...
void
updatewindowdesktop(Client *c)
{
    int i = 0;
    unsigned int tags = c->tags;
    while (tags >> 1) { i++; tags >>= 1; }
    long data[] = { i };
